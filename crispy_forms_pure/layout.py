...
TEMPLATE_PACK = getattr(settings, 'CRISPY_TEMPLATE_PACK', 'pure')
...
class Column(Div):
    ...
    This is the column from the `Pure Grid`_, all columns should be
...
class InlineField(Field):
    """
    Layout object for rendering an inline field with Pure
...
class SwitchField(crispy_forms_layout.Field):
    """
    A specific field to use Pure form switches

    You should only use this with a checkbox field and this is a *raw* usage of this
    Pure element, you should see ``InlineSwitchField`` instead.
...
class InlineSwitchField(InlineField):
    """
    Like ``SwitchField`` it use Pure form switches with checkbox field but within an ``InlineField``

    Contrary to ``SwitchField`` this play nice with the label to be able to display it (as Pure form switches default behavior is to hide the label text)
